...
* **Синхронизация (Layout Persistence):** При перемещении или масштабировании виджетов срабатывает обработчик `onLayoutChange`, сопоставляющий изменённые координаты с массивом виджетов и инициирующий автоматическую отправку `PUT` запроса через TanStack Query мутацию без блокировки интерфейса.
